...
$$
P(X_j=X_j^{(test)}|Y=C_k)=\frac{m_{mj}^{test}}{m_k}
$$

其中，${m_k}$为样本类别为$C_k$的数目，$m_{mj}^{(test)}$为类别为$C_k$的样本，第j维特征$X_j^{(test)}$出现的次数。

某些时候，可能出现某些类别未出现在样本中，这样可能导致
$P(X_j=X_j^{(test)}\big|Y=C_k)=0$，为解决这个问题，我们引入拉普拉斯平滑，有：

$$
P(X_j=X_j^{(test)}|Y=C_k)=\frac{m_{mj}^{(test)}+\lambda}{m_k+O_j\lambda}
$$
